...
async def process_code_request(repo_url: str, prompt: str) -> AsyncGenerator[Dict[str, Any], None]:
    ...
    try:
        ...
        sandbox = Sandbox()
        logger.info(f"E2B sandbox initialized with ID: {sandbox.sandbox_id}")
        ...
        yield create_status_update("success", f"Read {len(existing_files)} files", "reading", 65)
        ...
        for edit in file_edits:
            try:
                filepath = edit["file_path"]  # Updated to match new format
                new_content = edit["new_content"]
                full_path = f"{repo_name}/{filepath}"
                
                # Ensure directory exists
                dir_path = "/".join(full_path.split("/")[:-1])
                if dir_path and not sandbox.files.exists(dir_path):
                ...
                logger.error(f"Failed to modify {edit.get('file_path', 'unknown')}: {str(e)}")
                yield create_status_update(
                    "warning", 
                    f"Failed to modify {edit.get('file_path', 'unknown')}: {str(e)}", 
                    "implementation"
                )
        ...
        try:
            pr_result = await github_pr_creator.create_pr_with_changes(
                repo_url=repo_url,
                prompt=prompt,
            ...
            if pr_result["success"]:
                ...
                yield create_status_update(
                    "warning", 
                    f"⚠️ PR creation failed: {pr_result['error']}", 
                    "complete", 
                    100
                )
        ...
        if sandbox:
            try:
                ...
                logger.info(f"Sandbox {sandbox.sandbox_id} closed successfully")
            except Exception as e:
